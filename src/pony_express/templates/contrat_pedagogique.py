...
from pony_express.service.utils import get_date_from_timestamp, group_blocks, clean_text
...
# Tables des blocs répétables : une ligne par bloc, reliée au dossier par son numéro
TABLE_ACQUIS = "Demarche_128447_repetable_acquis_d_apprentissage"
TABLE_ACTIVITES = "Demarche_128447_repetable_activites_et_taches"
...
EXTRA_TABLES = {
    ...
    TABLE_ACTIVITES: [BLOCK_KEY, BLOCK_ORDER, "activite", "tutorat_et_suivi_de_cette_activite"],
}
...
def apply_data_transformation(data: dict) -> dict:
    ...
    set_list(transformed_dict, STUDENT_DATA.acquis_list.name, [b.get("acquis") for b in acquis])
    set_list(transformed_dict, STUDENT_DATA.activites.name, [b.get("activite") for b in activites])
    set_list(transformed_dict, STUDENT_DATA.tutorings.name,
